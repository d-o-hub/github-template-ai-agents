...
NETWORK_KEYWORDS="${NETWORK_KEYWORDS:-curl:wget:nc:netcat:nmap:ssh:scp:sftp:rsync:socat:nslookup:dig:host:nc.openbsd:nc.traditional:telnet:ftp:tftp:ssh-add:ssh-agent:ncat:tcpdump:wireshark:tshark:aria2c:lynx:links:elinks:expect:rclone:ssh-copy-id:ssh-keyscan:ngrok:cloudflared:aws:gcloud:az:whois:s3cmd:gsutil:mc:http:wg:openvpn:netstat:ss:traceroute:tracepath}"
