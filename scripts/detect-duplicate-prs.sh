...
main() {
  ...
  # perf: replace expensive grep and comm subshells with native bash features
  local i j a b fname a_hash b_hash contained total=${#ordered[@]}
  local nl=$'\n' stripped_a count_a stripped_b count_b
  local -A set_b=()
  for ((i = 0; i < total; i++)); do
    for ((j = i + 1; j < total; j++)); do
      ...
      count_a=$(( ${#pr_files[$a]} == 0 ? 0 : ${#stripped_a} + 1 ))
      stripped_b="${pr_files[$b]//[^$nl]/}"
      count_b=$(( ${#pr_files[$b]} == 0 ? 0 : ${#stripped_b} + 1 ))
  ...
  # perf: replace process substitution, comm, and grep with native bash features
  local -a files_a=() files_b=()
  local overlap smaller f
  local -A overlap_set=()
  for ((i = 0; i < total; i++)); do
    for ((j = i + 1; j < total; j++)); do
      a="${ordered[$i]}"
      ...
      files_a=()
      [[ -n "${pr_files[$a]}" ]] && mapfile -t files_a <<< "${pr_files[$a]}"
      files_b=()
      [[ -n "${pr_files[$b]}" ]] && mapfile -t files_b <<< "${pr_files[$b]}"

      local -a valid_files_a=() valid_files_b=()
      for f in "${files_a[@]}"; do [[ -n "$f" ]] && valid_files_a+=("$f"); done
      for f in "${files_b[@]}"; do [[ -n "$f" ]] && valid_files_b+=("$f"); done

      (( ${#valid_files_a[@]} > 0 && ${#valid_files_b[@]} > 0 )) || continue

      overlap_set=()
      for f in "${valid_files_b[@]}"; do
        overlap_set["$f"]=1
      done

      overlap=0
      for f in "${valid_files_a[@]}"; do
        [[ -n "${overlap_set["$f"]:-}" ]] && overlap=$((overlap + 1))
      done

      (( overlap > 0 )) || continue
      smaller=$(( ${#valid_files_a[@]} < ${#valid_files_b[@]} ? ${#valid_files_a[@]} : ${#valid_files_b[@]} ))
