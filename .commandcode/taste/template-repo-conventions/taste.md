...
- For template repos, `VERSION=0.0.0` is intentional (consumer-side default); template's own version lives in `.template/CHANGELOG-TEMPLATE.md`. Confidence: 0.85
...
- New skills must be added to `.agents/skills/<name>/` AND symlinked to `.claude/skills/` via `./scripts/setup-skills.sh`. (Qwen Code reads `.agents/skills/` directly; per-skill symlinks to `.qwen/skills/` are removed. The path `.qwen/skills/` remains in `.gitignore` line 74.) Confidence: 0.85
