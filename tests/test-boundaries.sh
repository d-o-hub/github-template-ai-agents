...
test_cmd "shred /dev/sda" "$DANGEROUS"
test_cmd "mkfs.ext4 /dev/sda1" "$DANGEROUS"
test_cmd "reboot" "$DANGEROUS"

echo "Testing INTERPRETER_KEYWORDS boundaries..."
test_cmd "python3 script.py" "$DANGEROUS"
test_cmd "pipenv install" "$DANGEROUS"
test_cmd "poetry run" "$DANGEROUS"
test_cmd "uv run" "$DANGEROUS"
