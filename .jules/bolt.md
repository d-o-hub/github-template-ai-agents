...
## 2026-06-02 - [Eliminate subshells in validation loops]
**Learning:** Performing multiple iterations over `find` outputs, command strings, and regex checks inside bash `while read` loops using process substitution (`< <(...)`) or pipes (`| while read`) introduces subshell processing overhead. Native bash commands like `for file in $variable` or `for file in /path/*` avoid creating new processes, improving execution speed significantly (e.g. from ~2.26s down to ~1.3s in the `quality_gate.sh` and related suite).
**Action:** Prefer native bash globbing (`for var in dir/*`) over `find ... | while read`, and native word-splitting (`for line in $var`) or `mapfile` over `printf | while read` where lines don't contain spaces. Be cautious with `IFS` resets to preserve accurate text behavior.
