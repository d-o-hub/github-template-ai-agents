...
## 2026-07-24 - Comprehensive Pattern-Based Key, Password, and SSH Private Key Prefix Blocking

**Vulnerability:** Gaps in pattern-based path filtering left potential avenues for exposing key and credential formats (e.g., GPG/PGP keys, App Store Connect private keys, local password tables, and custom-named shell or application histories). Additionally, checking for private SSH keys using explicit prefixes like `id_rsa` or `id_ed25519` failed to cover customized SSH key formats or custom-named keys starting with `id_` (e.g. `id_backup_key`).
**Learning:** Static lists of SSH algorithm prefixes are prone to omissions. Relying on explicit file patterns for histories or credentials fails when new applications use different prefixes or customized names. Pattern checks should use general prefixes and suffixes (e.g., blocking `id_` prefixes while exempting public keys ending in `.pub`, and blocking suffix patterns like `_history`) to create a comprehensive and resilient security posture.
**Prevention:** In `validate_safe_path`, block any path component starting with `id_` (unless ending in `.pub`), and expand blocked extensions to cover a wider spectrum of cryptographic, keychain, credential, and database formats (including `.gpg`, `.pgp`, `.asc`, `.p8`, `.pkcs12`, `.passwd`, `.pwd`, `.htpasswd`, and `_history`).
