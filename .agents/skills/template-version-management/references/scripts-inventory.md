...
**Template-repo behavior:** Updates `README.md` to `version-0.0.0` (because `VERSION=0.0.0`). Does not touch `.template/CHANGELOG-TEMPLATE.md` (which is the template's own history).
...
**Purpose:** Bump the patch version, append a new entry to `.template/CHANGELOG-TEMPLATE.md`, and propagate.

**Inputs:**
- `VERSION` file (current version)
- `.template/CHANGELOG-TEMPLATE.md` (target file)
...
- Inserts `## [X.Y.Z] - YYYY-MM-DD` into `.template/CHANGELOG-TEMPLATE.md` after `## [Unreleased]`
...
**Consumer-repo behavior:** Reads consumer `VERSION`, appends to consumer's `CHANGELOG.md` (note: consumer would rename `.template/CHANGELOG-TEMPLATE.md` to `CHANGELOG.md` per the bootstrap flow).
...
- Both `VERSION` and `.template/CHANGELOG-TEMPLATE.md` are updated

## When NOT to Modify These Scripts

The scripts are designed for downstream consumers. **Do not change their semantics to read from `.template/CHANGELOG-TEMPLATE.md` instead of `VERSION`** — that would break every consumer repo using this template.

If a template-specific behavior is needed (e.g., a `propagate-template-version.sh` that reads the latest `## [X.Y.Z]` heading from `.template/CHANGELOG-TEMPLATE.md`), add a **new** script rather than modifying these.
