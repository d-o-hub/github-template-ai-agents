...
6. **Periodic Action Audit**: Re-audit all workflow actions every GitHub deprecation cycle or when Node runtime warnings appear in CI logs.
7. **Playwright System Deps on Cache Hit**: When caching Playwright browsers in CI, separate browser install from system deps. Use `npx playwright install` (gated on cache miss) for browser binaries, and `npx playwright install-deps` (gated on cache hit) for apt packages.
