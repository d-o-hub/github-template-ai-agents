...
| `agent-browser` | Browser automation CLI for AI agents. Use when the user need | Browser automation CLI for agents |
...
| `anti-ai-slop` | Apply this skill whenever the user wants to audit, fix, rede | Quality enforcement for UI, UX, and copy |
...
| `codacy-analysis-cli` | Uses the Codacy Analysis CLI to run local static analysis. | Quality |
| `codacy-cloud-cli` | Uses the Codacy Cloud CLI to query repositories, issues, security findings, etc. | Quality |
...
| `privacy-first` | Prevent email addresses and personal data from entering the | Prevent PII/email leaks in the codebase |
...
| `readme-best-practices` | Create, audit, and improve GitHub README.md files following | Expert guidance for GitHub documentation |
