...
- **LESSON-035 — Metrics JSONL Merge Conflicts**: Concurrent PRs often conflict on the tail of per-agent metrics files. Resolved via per-agent file isolation (`.agents/metrics/metrics-{agent}.jsonl`) and `merge=union` in `.gitattributes` for same-agent collisions. See `scripts/log-metric.sh`.
